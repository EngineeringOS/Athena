...
class AthenaBoundaryDescriptorResolverTest {
    ...
    fun `resolves valid passive boundary descriptors into deterministic order and rejects sovereign or operational descriptors`() {
        ...
        val source = AthenaBoundaryDescriptorSource(
            descriptorRoots = listOf(
                ...
                repoRoot.resolve("compiler/src/test/resources/boundary-descriptors/invalid-assumption"),
    ...
    fun `deduplicates repeated descriptor roots before validation`() {
        val repoRoot = resolveRepoRoot()
        val duplicatedPath = repoRoot.resolve("compiler/src/test/resources/boundary-descriptors/automationml-reference")
        val source = AthenaBoundaryDescriptorSource(
            descriptorRoots = listOf(
                duplicatedPath,
                duplicatedPath,
    ...
    fun `rejects malformed boundary csv fields duplicate descriptor ids and non-reference standards posture`() {
        ...
        val source = AthenaBoundaryDescriptorSource(
            descriptorRoots = listOf(
                repoRoot.resolve("compiler/src/test/resources/boundary-descriptors/malformed-exchange-forms"),
                repoRoot.resolve("compiler/src/test/resources/boundary-descriptors/duplicate-id-a"),
                repoRoot.resolve("compiler/src/test/resources/boundary-descriptors/duplicate-id-b"),
                repoRoot.resolve("compiler/src/test/resources/boundary-descriptors/automationml-outbound"),
            ),
        )
    ...
    fun `deduplicates descriptor roots by canonical directory identity on windows aliases`() {
        ...
        val repoRoot = resolveRepoRoot()
        val descriptorPath = repoRoot.resolve("compiler/src/test/resources/boundary-descriptors/automationml-reference")
    ...
    fun `loads utf8 boundary manifests without mangling descriptor identity`() {
        ...
        val source = AthenaBoundaryDescriptorSource(
            descriptorRoots = listOf(
                repoRoot.resolve("compiler/src/test/resources/boundary-descriptors/utf8-identity"),
            ),
        )
