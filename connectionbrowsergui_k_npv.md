...
[Connections](Glossary_o_verbindungen.md) can exist between unplaced [connection points](Glossary_o_anschluesse.md) ([functions](Glossary_o_funktionen.md)) and mixed between placed and unplaced connection points. These connections are reported just as connections between placed connection points.

In the Connections - <Project name> dialog you can connect placed and [unplaced functions](Glossary_o_nicht_platziertefunktionen.md) or connection points. To do this, [generate](Glossary_o_erzeugen.md) a new (unplaced) connection and specify the connection source and target in the connection [properties](Glossary_o_eigenschaften.md). The source and target have to be existing functions (placed or unplaced) in the project.
...
Unplaced connections also inherit properties from [potentials](Glossary_o_potenziale.md) and from the project. The potential, as with placed connections, is determined when updating the connections and can't be entered on the connection.

Unplaced connections can be created on a [component](Glossary_o_schaltzeichen.md) as follows:
...
- Two [placed functions](Glossary_o_platziertefunktionen.md) are connected via an unplaced connection (e.g., via the generation of connections in the Interconnect devices dialog).
...
[Generating Unplaced Connections](planningconnections_h_createconnections.md)
