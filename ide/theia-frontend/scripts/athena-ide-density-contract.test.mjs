...
test('Athena graph workbench keeps controls in the bottom dock and projection info in a top popover', () => {
    ...
    assert.match(source, /resolveAthenaGraphPrimaryProductSurface\(model\.supportedViews\)/);
    assert.match(source, /data-athena-product-surface-id/);
    assert.match(source, /primarySurface\.displayName/);
    assert.doesNotMatch(source, /visibleProjectionViews\.map/);
    ...
    assert.doesNotMatch(source, /lastDocumentSheetViewSelector/);
    assert.doesNotMatch(source, /rememberDocumentSheetViewSelector\(model\)/);
