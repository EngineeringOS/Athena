...
const ATHENA_M40_PRODUCT_PROOF_SENTINEL = 'ATHENA_M40_PRODUCT_PROOF=';
const ATHENA_JAVA_SENTINEL = 'ATHENA_JAVA_HOME=';
const ATHENA_JAVA_UNRESOLVED_SENTINEL = 'ATHENA_JAVA_HOME_UNRESOLVED=';
const STARTUP_TIMEOUT_MS = 120000;
const SOURCE_RELATIVE = 'src/com/engineeringood/m40/rollingshutter/01-rolling-shutter-control.athena';

async function main() {
    const repositoryRoot = resolveM40Example();
    const screenshotPaths = resolveM40ScreenshotPaths();
    ...
    const productProof = buildM40ProductProof(repositoryRoot, smokes);
    assertM40ProductProof(productProof);

    console.log(`${ATHENA_M40_PRODUCT_PROOF_SENTINEL}${JSON.stringify(productProof)}`);
    console.log(`Athena M40 product proof passed. workspace=${smokes[0].openedWorkspace} screenshots=${Object.values(screenshotPaths).join(',')}`);
}
...
async function runElectronSmoke(repositoryRoot, screenshotPath, viewportName, windowWidth, windowHeight) {
    ...
    if (!sawWindowCreated || !sawReady || exitCode !== 0) {
        fail('electron-runtime', `M40 smoke failed. windowCreated=${sawWindowCreated} ready=${sawReady} exitCode=${exitCode}`, outputLines);
    }
    if (process.platform === 'win32' && !resolvedJavaHome && unresolvedJavaSignal) {
        fail('java-runtime', `M40 smoke did not resolve Java runtime: ${unresolvedJavaSignal}`);
    }
    ...
    if (!graphWorkbenchProof) {
        fail('graph-workbench-proof', 'M40 smoke did not report graph workbench proof.', outputLines);
    }
    return { viewportName, windowWidth, windowHeight, screenshotPath, openedWorkspace, graphWorkbenchProof, capturedScreenshotPath };
}

function buildM40ProductProof(repositoryRoot, smokes) {
    const primaryProof = smokes[0].graphWorkbenchProof || {};
    return {
        schemaVersion: 'm40.product-proof',
...
function assertM40ProductProof(proof) {
    requireValue(proof.schemaVersion === 'm40.product-proof', 'schema', 'Unexpected M40 proof schema.', proof);
...
function assertScreenshotProof(screenshots) {
    requireValue(Array.isArray(screenshots) && screenshots.length === 2, 'screenshot', 'Expected two M40 screenshots.', screenshots);
...
function resolveM40Example() {
    const repositoryRoot = path.resolve(__dirname, '..', '..', '..', 'examples', 'm40', 'rolling-shutter-control');
    ...
    if (missing.length > 0) {
        fail('repository-contract', `M40 product proof example missing: ${missing.join(', ')}`);
    }
    return repositoryRoot;
}

function resolveM40ScreenshotPaths() {
    const screenshotRoot = path.resolve(resolveRepoRoot(), '_bmad-output', 'implementation-artifacts', 'm40', 'screenshots');
    fs.mkdirSync(screenshotRoot, { recursive: true });
    return {
        desktop1920: path.join(screenshotRoot, 'm40-rolling-shutter-control-desktop-1920x1080.png'),
        narrow: path.join(screenshotRoot, 'm40-rolling-shutter-control-narrow.png'),
    };
}
...
function assertInstalledLspHostPresent() {
    ...
    if (!fs.existsSync(launcher)) {
        fail('lsp-installation', `M40 smoke requires installed LSP host. Run sequentially: .\\gradlew.bat --no-daemon --console=plain :ide:lsp:installDist`);
    }
}
...
module.exports = {
    buildM40ProductProof,
    assertM40ProductProof,
};
