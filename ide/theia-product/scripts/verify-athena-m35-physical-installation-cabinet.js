...
const ATHENA_CABINET_PRODUCT_PROOF_SENTINEL = 'ATHENA_M35_CABINET_PRODUCT_PROOF=';
const ATHENA_JAVA_SENTINEL = 'ATHENA_JAVA_HOME=';
const ATHENA_JAVA_UNRESOLVED_SENTINEL = 'ATHENA_JAVA_HOME_UNRESOLVED=';
const STARTUP_TIMEOUT_MS = 120000;
const SOURCE_RELATIVE = 'src/com/engineeringood/m35/physicalinstallationcabinet/01-physical-installation-cabinet.athena';
...
async function main() {
    const repositoryRoot = resolveM35SampleProject();
    const screenshotPaths = resolveM35ScreenshotPaths();
    ...
    console.log(`Athena M35 Cabinet product smoke passed. workspace=${smokes[0].openedWorkspace} screenshots=${Object.values(screenshotPaths).join(',')}`);
}
...
async function runElectronSmoke(repositoryRoot, screenshotPath, viewportName, windowWidth, windowHeight, openSourceForDiagnostics) {
    ...
    if (!sawWindowCreated || !sawReady || exitCode !== 0) {
        fail('electron-runtime', `M35 smoke failed. windowCreated=${sawWindowCreated} ready=${sawReady} exitCode=${exitCode}`, outputLines);
    }
    if (process.platform === 'win32' && !resolvedJavaHome && unresolvedJavaSignal) {
        fail('java-runtime', `M35 smoke did not resolve Java 25: ${unresolvedJavaSignal}`);
    }
    ...
    if (!graphWorkbenchProof) {
        fail('graph-workbench-proof', 'M35 smoke did not report graph workbench proof.', outputLines);
    }
    if (
        ...
        fail('lsp-diagnostics', `M35 smoke did not prove zero LSP diagnostics for ${SOURCE_RELATIVE}.`, outputLines);
    }
...
function buildCabinetProductProof(repositoryRoot, graphWorkbenchProof, screenshotEvidenceProof) {
    return {
        schemaVersion: 'm35.cabinet-product.v1',
...
function buildTraceProof(graphWorkbenchProof) {
    ...
    return {
        schemaVersion: 'm35.trace-proof.v1',
...
function assertTraceProof(proof) {
    requireValue(proof, 'trace-authority', 'Missing M35 trace proof.');
    requireValue(proof.requiredIdentitiesPresent === true, 'trace-authority', 'Rendered sample identities do not match M35 sample.', proof);
...
function buildCabinetAuthorityProof(repositoryRoot, graphWorkbenchProof) {
    ...
    return {
        schemaVersion: 'm35.cabinet-authority-proof.v1',
...
function assertCabinetAuthorityProof(proof) {
    requireValue(proof, 'cabinet-authority', 'Missing M35 Cabinet authority proof.');
    requireValue(proof.xmlRuntimeAuthorityAbsent === true, 'cabinet-authority', 'M35 sample still contains XML authority.', proof);
    requireValue(proof.rawMarkupAuthorityAbsent === true, 'cabinet-authority', 'M35 sample still contains raw HTML/QET authority.', proof);
    requireValue(proof.fallbackAuthorityAbsent === true, 'cabinet-authority', 'M35 runtime accepted fallback authority.', proof);
    requireValue(proof.physicalSourcePresent === true, 'cabinet-authority', 'M35 physical installation source missing.', proof);
    requireValue(proof.packageDescriptorPresent === true, 'cabinet-authority', 'M35 package descriptor missing.', proof);
    requireValue(proof.packageLockPresent === true, 'cabinet-authority', 'M35 package lock missing.', proof);
    requireValue(Number(proof.athenaRepresentationSourceCount) >= 2, 'cabinet-authority', 'M35 requires governed Athena representation sources.', proof);
    requireValue(Number(proof.governedSvgResourceCount) >= 1, 'cabinet-authority', 'M35 vendor SVG resource is missing.', proof);
}

function buildScreenshotEvidenceProof(smokes) {
    return {
        schemaVersion: 'm35.screenshot-evidence.v1',
...
function assertScreenshotEvidenceProof(proof) {
    ...
    requireValue(screenshots.length === 3, 'electron-screenshot', 'Expected three M35 screenshot evidence records.', proof);
...
function assertSourceDiagnosticActivationProof(proof) {
    ...
    requireValue(
        ...
        'lsp-diagnostics',
        'M35 smoke did not request or skip source diagnostic activation deterministically.',
        proof,
    );
    if (proof.requested === true) {
        requireValue(proof.opened === true, 'lsp-diagnostics', 'M35 smoke did not open the governed Athena source editor.', proof);
        requireValue(proof.problemMarkerCount === 0, 'lsp-diagnostics', 'M35 source has LSP problem markers.', proof);
        requireValue(proof.zeroProblemMarkerCount === true, 'lsp-diagnostics', 'M35 source did not prove zero problem markers.', proof);
    }
}

function resolveM35SampleProject() {
    const repositoryRoot = path.resolve(__dirname, '..', '..', '..', 'examples', 'm35', 'physical-installation-cabinet');
    const requiredFiles = [
        'athena.yaml',
        'athena.lock',
        SOURCE_RELATIVE,
        path.join('packages', 'representation', 'com', 'engineeringood', 'm35', 'standard', 'iec', 'standard-elements.athena'),
        path.join('packages', 'representation', 'com', 'engineeringood', 'm35', 'vendor', 'abb', 'pfea112', 'vendor-elements.athena'),
        path.join('packages', 'representation', 'com', 'engineeringood', 'm35', 'vendor', 'abb', 'pfea112', 'pfea112.svg'),
    ];
    const missing = requiredFiles.filter(filePath => !fs.existsSync(path.join(repositoryRoot, filePath)));
    if (missing.length > 0) {
        fail('repository-contract', `M35 physical installation Cabinet project missing: ${missing.join(', ')}`);
    }
    return repositoryRoot;
}

function resolveM35ScreenshotPaths() {
    const screenshotRoot = path.resolve(resolveRepoRoot(), '_bmad-output', 'implementation-artifacts', 'm35', 'screenshots');
    fs.mkdirSync(screenshotRoot, { recursive: true });
    return {
        desktop1920: path.join(screenshotRoot, 'm35-cabinet-product-smoke-desktop-1920x1080.png'),
        desktop1280: path.join(screenshotRoot, 'm35-cabinet-product-smoke-desktop-1280x900.png'),
        narrow: path.join(screenshotRoot, 'm35-cabinet-product-smoke-narrow.png'),
    };
}
...
function assertInstalledLspHostPresent() {
    ...
    if (!fs.existsSync(launcher)) {
        fail('lsp-installation', `M35 smoke requires installed LSP host. Missing ${launcher}. Run sequentially: .\\gradlew.bat --no-daemon --console=plain :ide:lsp:installDist`);
    }
}
