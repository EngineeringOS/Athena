...
async function main() {
    const repositoryRoot = resolveM22SampleProject();
    ...
    if (!sawWindowCreated || !sawReady || exitCode !== 0) {
        throw new Error(
            `Athena M22 sample project smoke failed. windowCreated=${sawWindowCreated} ready=${sawReady} exitCode=${exitCode}\n${outputLines.join('\n')}`
        );
    }
    if (process.platform === 'win32' && !resolvedJavaHome && unresolvedJavaSignal) {
        throw new Error(`Athena M22 sample project smoke did not resolve Java 25: ${unresolvedJavaSignal}`);
    }
    if (openedWorkspace !== repositoryRoot) {
        ...
        throw new Error(
            `Athena M22 sample project smoke opened '${openedWorkspace || 'n/a'}' instead of '${repositoryRoot}'.${failureLine ? `\n${failureLine}` : ''}`
        );
    }
    if (!graphWorkbenchProof) {
        throw new Error(`Athena M22 sample project smoke did not report graph workbench DOM proof.\n${outputLines.join('\n')}`);
    }
    ...
    if (missingGraphProof.length > 0) {
        throw new Error(
            `Athena M22 graph-workbench DOM proof failed: ${missingGraphProof.join(', ')}\n${JSON.stringify(graphWorkbenchProof)}`
        );
    }

    console.log(`${ATHENA_GRAPH_WORKBENCH_PROOF_SENTINEL}${JSON.stringify(graphWorkbenchProof)}`);
    console.log('Athena M22 graph-workbench DOM proof passed.');
    console.log(`Athena M22 sample project smoke passed. workspace=${openedWorkspace} javaHome=${resolvedJavaHome || 'n/a'}`);
}

function resolveM22SampleProject() {
    const repositoryRoot = path.resolve(__dirname, '..', '..', '..', 'examples', 'm22', 'sample-project');
    const requiredFiles = [
        'athena.yaml',
        'athena.lock',
        'README.md',
        path.join('src', '01-baseline-sheet.athena'),
        path.join('src', '02-layout-optimization-acceptance.athena'),
        path.join('src', '03-component-round-trip.athena'),
    ...
    if (missing.length > 0) {
        throw new Error(
            `Athena M22 sample project could not resolve the checked-in proof repository at ${repositoryRoot}. Missing: ${missing.join(', ')}`
        );
    }
