...
async function main() {
    const repositoryRoot = resolveM24SampleProject();
    ...
    if (!sawWindowCreated || !sawReady || exitCode !== 0) {
        throw new Error(
            `Athena M24 sample project smoke failed. windowCreated=${sawWindowCreated} ready=${sawReady} exitCode=${exitCode}\n${outputLines.join('\n')}`
        );
    }
    if (process.platform === 'win32' && !resolvedJavaHome && unresolvedJavaSignal) {
        throw new Error(`Athena M24 sample project smoke did not resolve Java 25: ${unresolvedJavaSignal}`);
    }
    if (openedWorkspace !== repositoryRoot) {
        ...
        throw new Error(
            `Athena M24 sample project smoke opened '${openedWorkspace || 'n/a'}' instead of '${repositoryRoot}'.${failureLine ? `\n${failureLine}` : ''}`
        );
    }
    if (!graphWorkbenchProof) {
        throw new Error(`Athena M24 sample project smoke did not report graph workbench DOM proof.\n${outputLines.join('\n')}`);
    }
    ...
    if (missingGraphProof.length > 0) {
        throw new Error(
            `Athena M24 graph-workbench DOM proof failed: ${missingGraphProof.join(', ')}\n${JSON.stringify(graphWorkbenchProof)}`
        );
    }
    ...
    console.log('Athena M24 route DOM proof passed.');
    console.log(`Athena M24 sample project smoke passed. workspace=${openedWorkspace} javaHome=${resolvedJavaHome || 'n/a'}`);
}

function assertRouteProof(routeProof) {
    if (!routeProof) {
        throw new Error('Athena M24 smoke did not receive routeProof in graph-workbench proof payload.');
    }
    const failures = [];
    ...
    if (failures.length > 0) {
        throw new Error(`Athena M24 route proof failed: ${failures.join('; ')}\n${JSON.stringify(routeProof, null, 2)}`);
    }
}
...
function assertInstalledLspHostPresent() {
    ...
    if (!fs.existsSync(launcher)) {
        throw new Error(
            `Athena M24 smoke requires the installed LSP host before product proof. Missing ${launcher}. Run: .\\gradlew.bat --no-daemon --console=plain :ide:lsp:installDist`
        );
    }
}

function resolveM24SampleProject() {
    const repositoryRoot = path.resolve(__dirname, '..', '..', '..', 'examples', 'm24', 'sample-project');
    const requiredFiles = [
        'athena.yaml',
        'athena.lock',
        'README.md',
        path.join('src', '01-control-route.athena'),
        path.join('src', '02-terminal-strip-routes.athena'),
        path.join('src', '03-power-protection-load.athena')
    ];
    const missing = requiredFiles.filter(filePath => !fs.existsSync(path.join(repositoryRoot, filePath)));
    if (missing.length > 0) {
        throw new Error(
            `Athena M24 sample project could not resolve the checked-in proof repository at ${repositoryRoot}. Missing: ${missing.join(', ')}`
        );
    }
