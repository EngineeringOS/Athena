...
`:extensions:domain-electrical` 模块是 Athena 的第一个真实领域插件。它通过核心拥有的插件契约封装 M0 Electrical/Runtime 的 lowering 与校验规则，用来证明领域含义可以存在于编译器核心之外。
...
该模块不拥有规范 IR 结构、编译器 pass 顺序、通用语义校验或插件发现机制。这些能力仍然由 `:kernel:compiler`、`:kernel:engineering-model` 与 `:kernel:validation` 核心拥有。
