# M22 Layout Hint Syntax Decision

Decision: layout block.

M22 selects a small project-level `layout` block for component placement, alignment, and grouping
round-trip. The block expresses engineering presentation intent as relationships. Raw pixel
coordinates are not the primary authored language.

Rule: raw pixel coordinates are not the primary authored language.

## Minimal Shape

```athena
layout schematic-sheet {
  place HMI1 near PLC1
  place XT1 below PLC1
  align HMI1 aligned-with PLC1 axis vertical
  group HMI1 grouped-with PLC1
}
```

## Supported M22 Intent

- place a component near another component
- place a component below another component
- align components with `aligned-with`
- group components with `grouped-with`

These map to the M22 layout constraint vocabulary: `near`, `below`, `aligned-with`, and
`grouped-with`.

## Deferred

Route and label persistence remains deferred. M22 may use route and label facts for readability, but
approved source round-trip in this milestone is limited to component placement, alignment, and
grouping.
