...
const ATHENA_M37_PRODUCT_PROOF_SENTINEL = 'ATHENA_M37_PROFESSIONAL_CONTROL_DRAWING_PRODUCT_PROOF=';
...
const SOURCE_RELATIVE = 'src/com/engineeringood/m37/professionalcontroldrawing/01-professional-control-drawing.athena';
const EXPECTED_ROUTE_COUNT = 11;

async function main() {
    const repositoryRoot = resolveM37SampleProject();
    const screenshotPaths = resolveM37ScreenshotPaths();
    assertInstalledLspHostPresent();

    const smokes = [
        await runElectronSmoke(repositoryRoot, screenshotPaths.desktop1920, 'desktop-1920x1080', 1920, 1080, true),
    ...
    const productProof = buildM37ProductProof(repositoryRoot, smokes[0].graphWorkbenchProof, screenshotEvidenceProof);
    assertM37ProductProof(productProof);

    console.log(`${ATHENA_M37_PRODUCT_PROOF_SENTINEL}${JSON.stringify(productProof)}`);
    console.log(`${ATHENA_GRAPH_WORKBENCH_PROOF_SENTINEL}${JSON.stringify(smokes[0].graphWorkbenchProof)}`);
    console.log(`Athena M37 professional Control Drawing product smoke passed. workspace=${smokes[0].openedWorkspace} screenshots=${Object.values(screenshotPaths).join(',')}`);
}
...
async function runElectronSmoke(repositoryRoot, screenshotPath, viewportName, windowWidth, windowHeight, openSourceForDiagnostics) {
    ...
    if (!sawWindowCreated || !sawReady || exitCode !== 0) {
        fail('electron-runtime', `M37 smoke failed. windowCreated=${sawWindowCreated} ready=${sawReady} exitCode=${exitCode}`, outputLines);
    }
    if (process.platform === 'win32' && !resolvedJavaHome && unresolvedJavaSignal) {
        fail('java-runtime', `M37 smoke did not resolve Java 25: ${unresolvedJavaSignal}`);
    }
    ...
    if (!graphWorkbenchProof) {
        fail('graph-workbench-proof', 'M37 smoke did not report graph workbench proof.', outputLines);
    }
    if (
        ...
        fail('lsp-diagnostics', `M37 smoke did not prove zero LSP diagnostics for ${SOURCE_RELATIVE}.`, outputLines);
    }
    return { viewportName, windowWidth, windowHeight, screenshotPath, openedWorkspace, graphWorkbenchProof, capturedScreenshotPath };
}

function buildM37ProductProof(repositoryRoot, graphWorkbenchProof, screenshotEvidenceProof) {
    ...
    return {
        schemaVersion: 'm37.professional-control-drawing-product.v1',
...
function assertM37ProductProof(proof) {
...
function assertProfessionalTraceProof(proof) {
    requireValue(proof.occurrenceCount > 0, 'trace-authority', 'No M37 graphic occurrence traces rendered.', proof);
    requireValue(proof.tracedOccurrenceCount === proof.occurrenceCount, 'trace-authority', 'Some M37 occurrences lack source-backed trace.', proof);
    requireValue(proof.routeTraceCount >= EXPECTED_ROUTE_COUNT, 'trace-authority', 'Missing M37 route trace records.', proof);
    requireValue(proof.routeSourceSpanCount === proof.routeTraceCount, 'trace-authority', 'Some M37 routes lack sourceSpan.', proof);
    requireValue(proof.packageResourceIds.length > 0, 'trace-authority', 'Missing packageResourceIds for M37 trace.', proof);
...
function assertRouteEvidenceProof(proof) {
    requireValue(proof.routeCount >= EXPECTED_ROUTE_COUNT, 'route-evidence', `Expected at least ${EXPECTED_ROUTE_COUNT} M37 routes.`, proof);
    requireValue(proof.acceptedRouteCount === proof.routeCount, 'route-evidence', 'Some visible routes lack M37 route evidence.', proof);
...
function buildAuthorityProof(repositoryRoot, graphWorkbenchProof) {
    ...
    return {
        schemaVersion: 'm37.authority-proof.v1',
...
function assertAuthorityProof(proof) {
    requireValue(proof.rawMarkupAuthorityAbsent === true, 'authority', 'M37 sample contains raw XML/SVG/QET/HTML authority files.', proof);
    requireValue(proof.xmlRuntimeAuthorityAbsent === true, 'authority', 'M37 sample contains XML runtime authority.', proof);
    requireValue(proof.fallbackAuthorityAbsent === true, 'authority', 'M37 runtime accepted fallback authority.', proof);
    ...
    requireValue(proof.sourcePresent === true, 'authority', 'M37 source file missing.', proof);
    requireValue(proof.packageDescriptorPresent === true, 'authority', 'M37 package descriptor missing.', proof);
    requireValue(proof.packageLockPresent === true, 'authority', 'M37 package lock missing.', proof);
    requireValue(Number(proof.athenaSourceCount) >= 4, 'authority', 'M37 requires Athena package sources.', proof);
}
...
function buildScreenshotEvidenceProof(smokes) {
    return {
        schemaVersion: 'm37.screenshot-evidence.v1',
...
function assertScreenshotEvidenceProof(proof) {
    ...
    requireValue(screenshots.length === 3, 'electron-screenshot', 'Expected three M37 screenshot evidence records.', proof);
...
function assertRouteProof(proof) {
    requireValue(proof && Number(proof.routeCount || 0) >= EXPECTED_ROUTE_COUNT, 'route-anchors', 'Missing M37 route proof.', proof);
...
function assertSourceDiagnosticActivationProof(proof) {
    ...
    requireValue(proof.requested === true || proof.skipped === true, 'lsp-diagnostics', 'M37 diagnostic activation state missing.', proof);
    if (proof.requested === true) {
        requireValue(proof.opened === true, 'lsp-diagnostics', 'M37 source editor did not open.', proof);
        requireValue(proof.problemMarkerCount === 0, 'lsp-diagnostics', 'M37 source has LSP problem markers.', proof);
        requireValue(proof.zeroProblemMarkerCount === true, 'lsp-diagnostics', 'M37 source did not prove zero problem markers.', proof);
    }
}

function resolveM37SampleProject() {
    const repositoryRoot = path.resolve(__dirname, '..', '..', '..', 'examples', 'm37', 'professional-control-drawing');
    const requiredFiles = [
        'athena.yaml',
        'athena.lock',
        SOURCE_RELATIVE,
        path.join('packages', 'representation', 'com', 'engineeringood', 'm37', 'professional', 'drawing-profile.athena'),
        path.join('packages', 'representation', 'com', 'engineeringood', 'm37', 'professional', 'm37-surface-bindings.athena'),
        path.join('packages', 'representation', 'com', 'engineeringood', 'm37', 'professional', 'm37-surface-elements.athena'),
    ];
    const missing = requiredFiles.filter(filePath => !fs.existsSync(path.join(repositoryRoot, filePath)));
    if (missing.length > 0) {
        fail('repository-contract', `M37 professional Control Drawing project missing: ${missing.join(', ')}`);
    }
    return repositoryRoot;
}

function resolveM37ScreenshotPaths() {
    const screenshotRoot = path.resolve(resolveRepoRoot(), '_bmad-output', 'implementation-artifacts', 'm37', 'screenshots');
    fs.mkdirSync(screenshotRoot, { recursive: true });
    return {
        desktop1920: path.join(screenshotRoot, 'm37-professional-control-drawing-desktop-1920x1080.png'),
        desktop1280: path.join(screenshotRoot, 'm37-professional-control-drawing-desktop-1280x900.png'),
        narrow: path.join(screenshotRoot, 'm37-professional-control-drawing-narrow.png'),
    };
}
...
function assertInstalledLspHostPresent() {
    ...
    if (!fs.existsSync(launcher)) {
        fail('lsp-installation', `M37 smoke requires installed LSP host. Missing ${launcher}. Run sequentially: .\\gradlew.bat --no-daemon --console=plain :ide:lsp:installDist`);
    }
}
...
module.exports = {
    buildM37ProductProof,
    assertM37ProductProof,
