...
`:semantics-core` 模块拥有针对规范 `Engineering IR` 的通用语义校验能力。它为核心级而非领域级规则提供确定性的诊断与继续执行策略。
...
该模块不解析源文本、不负责 AST lowering、不定义 Electrical/Runtime 兼容性等领域规则，也不产生渲染输出。领域语义属于插件，编排属于 `:compiler`。
