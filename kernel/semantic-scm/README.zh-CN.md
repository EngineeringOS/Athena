...
该模块发布 baseline-loading seam，以及 deterministic semantic diff、consequence、review-summary、review-enrichment、commit-intent 与 package-aware history 契约和 generator，但它仍然不实现 vendor substrate access、commit execution、hosted-plugin enrichment orchestration、LSP / Theia SCM transport、registry workflow 或 remote publish transport。它保持 JVM-first、VCS-neutral，并聚焦在语义契约与少量编排辅助之上。
