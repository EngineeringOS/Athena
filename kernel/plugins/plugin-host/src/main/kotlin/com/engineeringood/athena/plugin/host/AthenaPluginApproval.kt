...
/** Validates plugin manifests and directly-instantiated plugin objects against the hosted core contract. */
...
/** Approval-layer service that governs activation of source-enumerated plugin candidates. */
class AthenaPluginApprovalService(
    ...
    /** Validates candidates, rejects duplicates and incompatibilities, and returns the approved inventory. */
    fun approve(
        candidates: List<AthenaPluginCandidate>,
        rejectedCandidates: List<RejectedAthenaPluginCandidate> = emptyList(),
    ): AthenaPluginApprovalResult {
        val duplicatePluginIds = candidates.groupBy { it.manifest.pluginId }
            .filterValues { it.size > 1 }
            .keys

        val approvedPlugins = mutableListOf<ApprovedAthenaPlugin>()
        val allRejectedCandidates = rejectedCandidates.toMutableList()
        ...
        candidates.forEach { candidate ->
            ...
            if (diagnostics.isEmpty()) {
                ...
                allRejectedCandidates += RejectedAthenaPluginCandidate.fromCandidate(
                    candidate = candidate,
        ...
        return AthenaPluginApprovalResult(
            runtime = runtime,
            rejectedCandidates = allRejectedCandidates.sortedWith(rejectedPluginComparator),
