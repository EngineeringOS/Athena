...
class AthenaKnowledgeResolverTest {
    ...
    fun `resolves valid governed knowledge packages into deterministic active context order`() {
        ...
        val source = AthenaKnowledgePackageSource(
            packageRoots = listOf(
                ...
                repoRoot.resolve("kernel/compiler/src/test/resources/knowledge-packages/valid-standards-mapping"),
            ),
        )
    ...
    fun `rejects invalid or incompatible governed knowledge packages before they become active`() {
        ...
        val source = AthenaKnowledgePackageSource(
            packageRoots = listOf(
                ...
                repoRoot.resolve("kernel/compiler/src/test/resources/knowledge-packages/malformed-manifest"),
            ),
        )
    ...
    fun `deduplicates repeated governed knowledge package roots before activation`() {
        val repoRoot = resolveRepoRoot()
        val duplicatedRulePath = repoRoot.resolve("kernel/compiler/src/test/resources/knowledge-packages/valid-rule")
        val source = AthenaKnowledgePackageSource(
            packageRoots = listOf(
                duplicatedRulePath,
                duplicatedRulePath,
