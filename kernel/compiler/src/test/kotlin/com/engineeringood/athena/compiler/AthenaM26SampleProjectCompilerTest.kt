...
class AthenaM26SampleProjectCompilerTest {
    @Test
    fun `m26 sample project source files compile without diagnostics`() {
        val compiler = AthenaCompiler()
        sampleProjectSources().forEach { source ->
            assertTrue(Files.exists(source), "Missing M26 sample source: $source")
    ...
    @Test
    fun `m26 sample project links authored references without ambiguity`() {
        val compiler = AthenaCompiler()
        val sampleProjectRoot = resolveRepoRoot().resolve("examples/m26/sample-project")
