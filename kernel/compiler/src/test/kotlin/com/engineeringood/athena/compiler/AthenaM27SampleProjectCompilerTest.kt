...
class AthenaM27SampleProjectCompilerTest {
    @Test
    fun `m27 sample project source files compile without diagnostics`() {
        val compiler = AthenaCompiler()
        sampleProjectSources().forEach { source ->
            assertTrue(Files.exists(source), "Missing M27 sample source: $source")
    ...
    @Test
    fun `m27 sample project links authored references without ambiguity`() {
        val compiler = AthenaCompiler()
        val sampleProjectRoot = resolveRepoRoot().resolve("examples/m27/sample-project")
    ...
    private fun sampleProjectSources(): List<Path> {
        ...
        return listOf(
            repoRoot.resolve("examples/m27/sample-project/src/com/engineeringood/m27/sample/01-workspace-semantic-source.athena"),
            repoRoot.resolve("examples/m27/sample-project/src/com/engineeringood/m27/sample/02-field-assets-not-a-sheet.athena"),
        )
    }
