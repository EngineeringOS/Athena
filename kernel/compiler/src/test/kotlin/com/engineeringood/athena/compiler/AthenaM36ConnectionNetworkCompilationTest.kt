...
class AthenaM36ConnectionNetworkCompilationTest {
    @Test
    fun `lowers grouped connections into a semantic network and junction fact`() {
        val path = Files.createTempFile("athena-m36-network-", ".athena")
        path.writeText(
            """
            system M36Network {
              device Source {
              ...
              connect supply_group {
                ...
                relay_supply Source.out -> BranchB.in
              }
            }
            """.trimIndent(),
        )
        ...
        try {
            ...
            assertTrue(result.semanticResult.diagnostics.none { it.ruleId.value.startsWith("connectable.network.") })
        } finally {
            path.deleteIfExists()
        }
    }
    ...
    fun `reports a network diagnostic when a grouped junction uses a one-way shared port`() {
        val path = Files.createTempFile("athena-m36-network-invalid-", ".athena")
        path.writeText(
            """
