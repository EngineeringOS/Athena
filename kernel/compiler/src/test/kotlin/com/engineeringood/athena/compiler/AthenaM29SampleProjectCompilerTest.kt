...
class AthenaM29SampleProjectCompilerTest {
    @Test
    fun `m29 sample project source files compile without diagnostics and expose projections`() {
        val compiler = AthenaCompiler()
        sampleProjectSources().forEach { source ->
            assertTrue(Files.exists(source), "Missing M29 sample source: $source")
            val sourceText = Files.readString(source)
            assertTrue(sourceText.contains("    port "), "M29 sample must use nested device-owned ports: $source")
            ...
            when (result) {
                is CompilerCompilationSuccess -> {
                    ...
                    assertTrue(result.projections.isNotEmpty(), "M29 sample projection must be available for $source")
                }
    ...
    @Test
    fun `m29 sample project links authored references without ambiguity`() {
        val compiler = AthenaCompiler()
        val sampleProjectRoot = resolveRepoRoot().resolve("examples/m29/sample-project")
