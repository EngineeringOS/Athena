...
class AthenaKnowledgePackageLoaderTest {
    ...
    fun `rejects malformed manifests with stable diagnostics and inspectable subjects`() {
        val malformedPath = resolveRepoRoot().resolve("kernel/compiler/src/test/resources/knowledge-packages/malformed-manifest")
    ...
    fun `rejects missing payload files deterministically`() {
        val missingPayloadPath = resolveRepoRoot().resolve("kernel/compiler/src/test/resources/knowledge-packages/missing-payload")
    ...
    fun `rejects unsupported artifact kinds and invalid compatibility ranges before operational use`() {
        val repoRoot = resolveRepoRoot()
        val invalidKindPath = repoRoot.resolve("kernel/compiler/src/test/resources/knowledge-packages/invalid-kind")
        val invalidCompatibilityPath = repoRoot.resolve("kernel/compiler/src/test/resources/knowledge-packages/invalid-compatibility")
    ...
    fun `does not treat authored dsl files or plugin-like layouts as governed knowledge packages`() {
        ...
        val pluginLikePath = repoRoot.resolve("kernel/compiler/src/test/resources/knowledge-packages/plugin-like-layout")
