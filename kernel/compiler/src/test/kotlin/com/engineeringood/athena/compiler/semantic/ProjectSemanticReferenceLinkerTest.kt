...
class ProjectSemanticReferenceLinkerTest {
    ...
    fun `reports unresolved endpoint references with stable diagnostics`() {
        ...
        val consumerDeclarations = declarations(
            "consumer.athena",
            "package com.root\nsystem Consumer {\n  connect missing_out_to_plc1_out Missing.out to PLC1.out\n}",
        )
    ...
    fun `reports ambiguous endpoint references without selecting by caller order`() {
        ...
        val consumerDeclarations = declarations(
            "consumer.athena",
            "package com.root\nsystem Consumer {\n  connect plc1_out_to_plc2_out PLC1.out to PLC2.out\n  port PLC2.out {}\n}",
        )
        val consumer = sourceUnit(rootKey, "consumer.athena", "consumer", consumerDeclarations)
    ...
    fun `reports ambiguous references across resolved governed package imports`() {
        ...
        val consumerAst = parse(
            ...
            system Consumer {
              connect shared_out_to_local_out Shared.out to Local.out
              port Local.out {}
            }
            """.trimIndent(),
        )
    ...
    private fun crossPackageFixture(importVendor: Boolean): CrossPackageFixture {
        ...
        val consumerAst = parse(
            ...
            buildString {
                ...
                appendLine("  connect vendor_out_to_local_out Vendor.out to Local.out")
