...
data class SchematicRouteIntent(
    ...
    init {
        ...
        require(sourceAnchor.subjectId == connectionRoleFact.sourceSubjectId) {
            "Route intent source anchor must match the source semantic subject."
        }
        require(sourceAnchor.portId == connectionRoleFact.sourcePortId) {
            "Route intent source anchor must match the source semantic port."
        }
        require(targetAnchor.subjectId == connectionRoleFact.targetSubjectId) {
            "Route intent target anchor must match the target semantic subject."
        }
        require(targetAnchor.portId == connectionRoleFact.targetPortId) {
            "Route intent target anchor must match the target semantic port."
        }
        require(constraints.all { constraint -> constraint.connectionId == connectionRoleFact.connectionId }) {
            "Route intent constraints must belong to the same semantic connection."
        }
    }

    internal fun stableKey(): String = listOf(
        connectionRoleFact.connectionId.value,
        routeId.value,
...
class SchematicRouteIntentProjector {
    fun project(
        ...
        sheetId: String,
        layoutContext: SchematicRoutingLayoutContext,
        connectionRoleFacts: List<ElectricalConnectionRoleFact>,
        anchors: List<TerminalAnchorFact>,
    ): SchematicRouteIntentSnapshot {
        val routeIntents = connectionRoleFacts.map { intent ->
    ...
    private fun defaultConstraints(intent: ElectricalConnectionRoleFact): List<RouteConstraint> {
        return listOf(
            RouteConstraint(
