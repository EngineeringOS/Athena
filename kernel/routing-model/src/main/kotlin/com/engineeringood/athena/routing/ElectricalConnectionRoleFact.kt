...
data class ElectricalConnectionRoleFact(
    ...
    val quality: ElectricalConnectionRoleQuality = ElectricalConnectionRoleQuality.satisfied(),
)
...
enum class ElectricalConnectionRoleQualityState {
    SATISFIED,
    DEGRADED,
}

/** Classification quality carried with intent so unsupported cases do not crash route derivation. */
data class ElectricalConnectionRoleQuality(
    val state: ElectricalConnectionRoleQualityState,
    val message: String? = null,
) {
    val isDegraded: Boolean
        get() = state == ElectricalConnectionRoleQualityState.DEGRADED
    ...
    companion object {
        fun satisfied(): ElectricalConnectionRoleQuality =
            ElectricalConnectionRoleQuality(ElectricalConnectionRoleQualityState.SATISFIED)

        fun degraded(message: String): ElectricalConnectionRoleQuality =
            ElectricalConnectionRoleQuality(ElectricalConnectionRoleQualityState.DEGRADED, message)
    }
}
...
data class ElectricalConnectionRoleInput(
    val connectionId: ElectricalConnectionId,
...
class ElectricalConnectionRoleClassifier {
    fun classify(input: ElectricalConnectionRoleInput): ElectricalConnectionRoleFact {
        ...
        val quality = if (role == ElectricalConnectionRole.UNKNOWN || signalClass == ElectricalSignalClass.UNKNOWN) {
            ElectricalConnectionRoleQuality.degraded(
                "Unsupported electrical connection intent for `${input.connectionId.value}`.",
            )
        } else {
            ElectricalConnectionRoleQuality.satisfied()
        }

        return ElectricalConnectionRoleFact(
    ...
    private fun classifyRole(
        input: ElectricalConnectionRoleInput,
        ...
        return when (signalClass) {
            ...
            } else if (isControlDirection(input.sourcePort.direction, input.targetPort.direction)) {
